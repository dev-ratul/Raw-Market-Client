...
const AllUsers = () => {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // 🔹 Real query trigger state

  // React Query with search param
  const {
    data: users = [],
    isLoading,
    refetch,
  } = useQuery({
    queryKey: ["all-users-search", searchQuery], // only triggers when searchQuery changes
    queryFn: async () => {
